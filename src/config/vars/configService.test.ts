...
const vars = {
  ...
  gcsStorageFile: 'GCS_STORAGE_FILE',
  gcsStorageId: 'GCS_STORAGE_ID',
  gcsStorageKey: 'GOOGLE_APPLICATION_CREDENTIALS',  
  nodeEnv: 'NODE_ENV',
  port: 'PORT',
  rootPath: 'ROOT_PATH',
  }
...
describe('ConfigService', () => {
  ...
  test('load NO da error de validación si estan configuradas las variables requeridas', () => {
    ...
    process.env[vars.rootPath] = 'some-path'
    process.env[vars.gcsBucket] = 'some-path'
    process.env[vars.gcsStorageId] = 'some-path'
    process.env[vars.gcsStorageKey] = 'some-path'
    process.env[vars.gcsStorageFile] = 'some-path'
